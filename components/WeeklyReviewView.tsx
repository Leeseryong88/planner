...
다음 지침을 반드시 지키세요:
- "doneThisWeek" 항목에는 각 업무가 완료된 날짜를 함께 표기하세요. 완료일 정보가 없으면 "${weekRanges.doneRange.end.toISOString().slice(0, 10)} (완료 예정)"처럼 이번 주 금요일 날짜 뒤에 "(완료 예정)"이라고 적어주세요.
- "nextWeekPlan" 항목에는 계획된 업무와 마감 임박 업무를 우선적으로 적으세요.
- "issues" 항목에는 다음 주 수행을 위해 필요한 지원이나 리스크를 적고, 없다면 반드시 "이슈 없음"이라고 적어주세요.

응답은 아래 JSON 형식 하나만 허용됩니다(설명 금지, JSON 이외의 내용 금지):
{
  "doneThisWeek": "\\n- 이번 주 완료 업무 요약",
  "nextWeekPlan": "\\n- 다음 주 계획 요약",
